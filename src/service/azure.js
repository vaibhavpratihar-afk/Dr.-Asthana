...
function getToken() {
  return execSync('ado-token', { encoding: 'utf-8', stdio: 'pipe' }).trim();
}
 ...
 *
 * Structure: context → approach → files → diff stats.
 */
function buildPRDescription(config, ticketKey, ticketSummary, cheatsheet, tmpDir) {
 ...
 */
export async function createPR(config, tmpDir, sourceBranch, targetBranch, ticketKey, ticketSummary, cheatsheet) {
  ...
  const description = buildPRDescription(config, ticketKey, ticketSummary, cheatsheet, tmpDir);
  ...
  try {
    const result = execSync(`az ${args.map(escapeArg).join(' ')}`, {
      ...
      timeout: AZ_TIMEOUT,
      encoding: 'utf-8',
      env: { ...process.env, AZURE_DEVOPS_EXT_PAT: getToken() },
    });
